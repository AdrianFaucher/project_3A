...
def display_hubble_MW(df:pd.DataFrame)->None:
    ...
    y_series = pd.to_numeric(df.loc[mask, 'V_h'], errors='coerce')
    
    plt.figure(figsize=(8, 5))
    # errors 
    e_x_min_series = pd.to_numeric(df.loc[mask, 'e_Dis_min'], errors='coerce')
    e_x_max_series = pd.to_numeric(df.loc[mask, 'e_Dis_max'], errors='coerce')
    e_y_series = pd.to_numeric(df.loc[mask, 'e_V_h'], errors='coerce')
    plt.errorbar(x_series, y_series, xerr=[e_x_min_series,e_x_max_series], yerr=e_y_series, fmt='none', ecolor='black', elinewidth=0.8, capsize=3, marker='x', markersize=8)
...
def display_velocities_distance(df, velocities: list[str], row_name: str, border: bool = False, colomn_name='Ra'):
    ...
    if len(dec_values_clean) > 0:
        norm = Normalize(vmin=np.min(dec_values_clean), vmax=np.max(dec_values_clean))
        colormap = cm.viridis  # Vous pouvez changer pour 'plasma', 'coolwarm', 'jet', etc.
    else:
    ...
    for i, ax in enumerate(axes):
        ...
        col_e_y_min = 'e_'+ velocities[i]+"_min_"+ row_name
        col_e_y_max = 'e_'+ velocities[i]+"_max_"+ row_name
        print(col_e_x_min, col_e_x_max, col_e_y_min, col_e_y_max)
        
        # Créer les couleurs pour chaque point
        colors = colormap(norm(dec_values))
        ...
        if col_e_y_max in df.columns and col_e_y_min in df.columns and col_e_x_min in df.columns and col_e_x_max in df.columns:
            ...
            # Pour errorbar avec couleurs, il faut tracer point par point
            for j in range(len(x)):
                if not (np.isnan(x[j]) or np.isnan(y[j]) or np.isnan(dec_values[j])):
                    ax.errorbar(x[j], y[j], 
                              xerr=[[e_x_min[j]], [e_x_max[j]]], 
                              yerr=[[e_y_min[j]], [e_y_max[j]]], 
                              color=colors[j], 
                              markersize=5, 
                              fmt='o', 
                              capsize=5, 
            ...
            scatter = ax.scatter(x, y, c=dec_values, cmap=colormap, norm=norm, 
        ...
        # Ajouter la barre de couleur pour chaque subplot
        if col_e_y_max in df.columns and col_e_y_min in df.columns and col_e_x_min in df.columns and col_e_x_max in df.columns:
            # Pour errorbar, créer une barre de couleur manuelle
            sm = cm.ScalarMappable(cmap=colormap, norm=norm)
            sm.set_array([])
            cbar = plt.colorbar(sm, ax=ax)
        else:
            # Pour scatter, utiliser la barre de couleur du scatter
            cbar = plt.colorbar(scatter, ax=ax)
        
        cbar.set_label('Dec (degrees)', rotation=270, labelpad=20)
...
def display_mean_squared_velocity_consistent(df,galaxy1,galaxy2, velocities, mass_ratios, partial_row_name, 
    ...
    if combined_plot:
        ...
        for i, velocity in enumerate(velocities):
            ...
            for mass_ratio in mass_ratios:
                ...
                mean_square = mean_square_peculiar_velocity_consistent(df, column,galaxy1, galaxy2, abnormal_galaxies)
