...
def identify_abnormal_galaxies_per_velocity(df, velocity, mass_ratios, partial_row_name, lower_bound, upper_bound, max_dix):
    ...
    for mass_ratio in mass_ratios:
        ...
        mask = (~df['Name'].str.startswith('CoM_'))  # Exclut les centres de masse et cenA et M83
...
def mean_square_peculiar_velocity_consistent(df, column,galaxy1, galaxy2, abnormal_galaxies):
    """Calcule la moyenne des carrés des vitesses en excluant les galaxies anormales"""
    mask = (
        ~df['Name'].str.startswith('CoM_')
    ) & (
        ~df['Name'].isin(abnormal_galaxies)
    ) & (
        ~df['Name'].isin([galaxy1, galaxy2])
    )
